$(document).ready(function() {
    ...
    $('a#PAOrder').click(function() {
      ...
      $('.search').html('');
      $('.table').load('PA-order-form.php', loadPAOrderFormEventHandler);
      return false;
    });
    /* New Bike order page */
    $('a.bikeOrder').click(function() {
      ...
      $('DIV.title').css('background', 'url("resources/images/colors/blue/title.png") repeat-x scroll 0 0 #336699');
      $('.search').html('');
      $('.table').load('bike-order-form.php', loadBikeOrderFormEventHandler);
      return false;
    });
    ...
    $('a#warranty').click(function() {
      ...
      $('DIV.title').css('background', 'url("resources/images/colors/blue/title.png") repeat-x scroll 0 0 #336699');
      $('.search').html('');
      $('.table').load('warranty-order-form.php', loadWarrantyFormEventHandler);
      return false;
    });
    ...
    $('a.adminSettings').click(function() {
      $('div.error').hide(); /* hide validation errors if coming from form page */
      $('.table').html('');
      $('.table').load('adminSettings.php?action='+ $(this).attr('id'), loadAdminSettings);
      return false;
    });
...
function loadBikeOrderFormEventHandler() {
    ...
    $('input[name="orderType"]').change(function() {
        if ($("input[name='orderType']:checked").val() == 'warranty') {
      $('span#RAnumber').slideDown('fast').css('display', 'block');
      $('input.part6num').css('background-color', '#F08080');
      $('th.part6num').html("Rpro #<br /><br /><span class='part6num'>Part # 6 required</span>");
        }
        else {
      $('span#RAnumber').slideUp('slow').css('display', 'none');
    ...
    /* Form Validations */
    function BikeFormInputsValid() {
      $.validity.start();
      //if ( $('#RAnumber').is(':visible') ) {
  // add RA number to required list
  //$("#RA_number").require();
      //}
      $('#name_first, #name_last, #day_phone, #store, #employee, #receipt_number, #RA_number').require();
      //$("#day_phone").require().match("phone");
      var result = $.validity.end();
      return result.valid;
    }

    /* Flag for unsaved changes if form is dirty and user leaves form */
    var isDirty = false;
    $('form#Bike_order_form:input').change(function() {
        if (!isDirty) {
            isDirty = true;
        }
    });
    ...
    $('#Bike_order_submit_button').click(function() {
          /* Validate form */
    $valid = true;
    //if ($valid) {
      /* data valid, so continue */
      var BikeDataForm = $('form#Bike_order_form').serialize();
      /* disable submit button */
      $('#Bike_order_submit_button').prop('disabled', 'disabled');
      $.ajax({
    type: 'POST',
    ...
    success: function(response) {
      ...
      //$('form#Bike_order_form').clearForm();
    }
      });
    //} else {
...
/*  $('.alpha').alpha(); */
  $('.alphanumeric_plus').alphanumeric({allow: '-.# '});
  $('#comments').alphanumeric({allow: '#.-!$%_ '});
  $('.numeric').numeric();
  $('.numeric_plus').numeric({allow: '-.'});
  $('.cc_numeric').numeric({allow: '-'});
  $('.date_numeric').numeric({allow: '-/'});
  $('.email').alphanumeric({allow: '.-_@'});
    ...
    $('#store').change(function() { // changes tax amount and recalculates
    ...
    $('#visa, #mastercard, #amex, #discover').click(function() {
      var cc = $(this);
      if (cc.value === 'amex') {
  $('#cust_cc_num4').attr({'maxlength': '3', 'size': '3'});
      }
    ...
    /* Flag for unsaved changes if form is dirty and user leaves form */
    var isDirty = false;
    $('form#PA_order_form:input').change(function() {
        if (!isDirty) {
            isDirty = true;
        }
    });
...
*/
    /* Submit button (Validates data before continuing submission) */
    $('#PA_order_submit_button').click(function() {
          /* Validate form */
    if (FormInputsValid()) {
      var dataForm = $('#PA_order_form').serialize();
      /* disable submit button */
    ...
    success: function(response) {
      ...
      //alert (response);
      $('#status').fadeIn('slow').html(response);
      //$('form#special_order_form').clearForm();
    }
      });
      return false;
    } else {
...
function loadWarrantyFormEventHandler() {
    ...
    function WarrantyFormInputsValid() {
      $.validity.start();
      $('#name_first, #name_last, #store, #employee, #RA_number').require();
      $('#day_phone').require().match('phone');
      var result = $.validity.end();
      return result.valid;
    }
    ...
    /* Flag for unsaved changes if form is dirty and user leaves form */
    var isDirty = false;
    $('form#warranty_order_form:input').change(function() {
        if (!isDirty) {
            isDirty = true;
        }
    });
...
*/
    /* Submit button (Validates data before continuing submission) */
    $('#warranty_order_submit_button').click(function() {
          /* Validate form */
    if (WarrantyFormInputsValid()) {
      var WarrantyDataForm = $('#warranty_order_form').serialize();
      /* disable submit button */
      $('#warranty_order_submit_button').prop('disabled', true);
      $.ajax({
    type: 'POST',
    url: 'specialOrders.php',
    data: WarrantyDataForm,
    success: function(response) {
      //$('#warranty_order_submit_button').css("background-color","#33FF00").attr('value', 'Success!');
      $('#status').fadeIn('slow').html(response);
      //$('form#warranty_order_form').clearForm();
    }
      });
    } else {
      alert('Make sure required fields are completed!');
...
function loadOrderStatusSearchForm() {
   $('#search_order_input_box').autocomplete({
      ...
      source: function(request, response) {
                  ...
                  $.getJSON('orderStatus.php', request, function(data) {
                     response(data);
                  });
      },
      select: function(event, ui) {
         ...
         var dateOrdered = (ui.item.date_ordered == '0000-00-00') ? 'Not yet ordered' : ui.item.date_ordered;
            ...
            var dateOrdered = (ui.item.others[x].date_ordered == '0000-00-00') ? 'Not yet ordered' : ui.item.others[x].date_ordered;
   ...
   $('#openOrderSearch').change(function() {
      $('#search_results').html('');
    $.getJSON('orderSearchbyStore.php', {store: $('#openOrderSearch').val()}, function(data) {
   ...
   $('td img', oTable.fnGetNodes()).each(function() {
      $(this).click(function() {
         ...
         else
         {
            ...
            /* Load data */
            $.getJSON('orderDetail.php?id=' + id, function(data) {
               ...
               $('input.editMyDetails', oTable.fnGetNodes()).editable('orderAdminUpdate.php', /* added oTable.fnGetNodes() */
               {
                  ...
                  submitdata: function(value, settings) {
                        return { 'id': $('input.editMyDetails').attr('item') };
                  },
...
function loadAdminSettings() {
   ...
   $('.editMe').editable('AdminSettingsUpdate.php',
   {
      ...
      submitdata: function(value, settings) {
                     return { 'table': this.parentNode.getAttribute('class'), 'id': this.parentNode.getAttribute('id'), 'field': this.getAttribute('data-field') };
         },
  ...
  $('form#add_status_code').find('input[name=submit]').click(function() {
      var dataForm = $('#add_status_code').serialize();
      $.ajax({
        type: 'POST',
        ...
        success: function(response) {
          ...
          $('tr#'+ response.id + ' > td').css('background-color', '#33FF00').animate({backgroundColor: backgroundColor},2500);
        }
      });
      return false;
  });
  ...
  $('a.delete_status_code').click(function() {
      $.ajax({
        type: 'POST',
        url: 'AdminSettingsUpdate.php',
        data: {action: 'delete_status_code', id: $(this).attr('id')},
        dataType: 'json',
        success: function(response) {
  ...
  $('a.delete_store').click(function() {
      $.ajax({
        type: 'POST',
        url: 'AdminSettingsUpdate.php',
        data: {action: 'delete_store', id: $(this).attr('id')},
        dataType: 'json',
        success: function(response) {
            $('tr#'+ response.id).css('background-color', '#FBE3E4').hide('slow');
        }
  ...
  $('a.delete_email_recipient').click(function() {
      $.ajax({
        type: 'POST',
        url: 'AdminSettingsUpdate.php',
        data: {action: 'delete_email_recipient', id: $(this).attr('id')},
        dataType: 'json',
        success: function(response) {
            $('tr#'+ response.id).css('background-color', '#FBE3E4').hide('slow');
        }
...
function fetchDetails(order_id) {
    $.getJSON('orderDetail.php', {id: order_id}, function(data) {
